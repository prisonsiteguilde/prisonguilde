...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  async openChest(chestId) {
    ...
    const state = get().state;
    if (state) set({ state: { ...state, player: result.player } });
    window.Telegram?.WebApp.hapticFeedback?.notificationOccurred("success");
  },
  async startCraft(recipeId) {
    const result = await api.startCraft(recipeId);
    const state = get().state;
    if (state) set({ state: { ...state, player: result.player, craftJobs: [...state.craftJobs, result.job] } });
  },
  ...
  updatePlayer(player) {
    const state = get().state;
    if (state) set({ state: { ...state, player } });
  }
}));
